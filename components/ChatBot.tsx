  ...
  const handleSend = async () => {
    ...
    const systemInstruction = `
       ...
       **التعامل مع الأحجام:**
       ...
       - مثال: "عندنا كورتادو بثلاث أحجام: صغير (15 ريال)، وسط (18 ريال)، كبير (20 ريال). أي حجم يناسبك؟"
       - عند استخدام [SUGGEST_PRODUCT:ID]، سيظهر للعميل خيارات الحجم تلقائياً عند الضغط على "إضافة للسلة".
      ...
      if (p.sizes && p.sizes.length > 0) {
        sizeInfo = `, الأحجام: ${p.sizes.map(s => `${s.name} (${p.price + s.priceModifier} ريال)`).join(' | ')}`;
      }
      return `- ${p.name} (ID: ${p.id}, قسم: ${category?.name || p.category}, السعر الأساسي: ${p.price} ريال${sizeInfo}, وصف: ${p.description || 'لا يوجد'})`;
    }).join('\n')}
    `;
