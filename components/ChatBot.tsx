  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (!isChatOpen) {
        ...
        setTimeout(() => setShowAutoPopup(false), 5000);
      }
    }, 30000); // 30 seconds
      ...
      {!isCartOpen && (
        <>
          ...
          <AnimatePresence>
            {showAutoPopup && !isChatOpen && (
              <motion.div
                ...
                className="fixed bottom-24 right-6 bg-white dark:bg-gray-800 text-gray-800 dark:text-white px-4 py-3 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 z-[99] max-w-[200px]"
              >
          ...
          <motion.button
            ...
            onClick={toggleChat}
            className="fixed bottom-6 right-6 p-4 rounded-full shadow-2xl text-white z-[100] flex items-center justify-center group relative"
            style={{ backgroundColor: settings.primaryColor }}
          >
